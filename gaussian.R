...
singlecellplot <- function(outlinedata, spotdata,framenum, cellnum){
  ...
  for(n in 1:nrow(so)){
    dat <- circleFun(c(so$Lmid[n],so$Dum[n]),so$fwhm_x[n]*0.064, so$fwhm_y[n]*0.064,npoints = 100, so$color[n])
  ...
  return(ggplot(oo, aes(x=y0_rot*-0.064, y=x0_rot*-0.064)) +geom_point() + geom_point(aes(x=y1_rot*-0.064, y=x1_rot*-0.064)) + geom_point(data=so, aes(x=Lmid, y=Dum, color=color)) + geom_polygon(data=totalframe, aes(x=x, y=y, fill= color, linetype=as.factor(Lmid)), alpha=0.4) + theme_minimal() + scale_colour_manual(
...
circleFun <- function(center = c(0,0),diameterx = 1, diametery = 1, npoints = 100, color1){
  rx = diameterx / 2
  ry = diametery /2
...
groupdists <- function(comdat){
  ...
  comdat$group[comdat$sdist < (comdat$fwhm_x_GFP + comdat$fwhm_x_RFP)*0.064] <- "< fwhm1+fwhm2"
  comdat$group[comdat$sdist < comdat$fwhm_x_GFP*0.064|comdat$sdist < comdat$fwhm_x_RFP*0.064] <- "< fwhm"
  return(comdat)
}
...
timecellplot <- function(dat, num, Mdat){
  return(ggplot(dat[dat$cell==num,], aes(x=Lmid, y=Dum, color=frame)) + geom_point() + scale_colour_gradient2(low = "#CC79A7", mid= "#F0E442", high = "#009E73", midpoint = 30, space = "Lab", guide = "colourbar") + theme_minimal() 
         + geom_point(data=Mdat[Mdat$cell==num&Mdat$slice==1,], aes(x=y0_rot*-0.064, y=x0_rot*-0.064), colour="black") + geom_point(data=Mdat[Mdat$cell==num&Mdat$slice==1,], aes(x=y1_rot*-0.064, y=x1_rot*-0.064),colour="black"))
}
...
timecelllineplot <- function(dat, num, Mdat){
  return(ggplot(dat[dat$cell==num,], aes(x=Lmid, y=Dum, color=frame)) + geom_path(aes(line=as.factor(spot))) + scale_colour_gradient2(low = "#CC79A7", mid= "#F0E442", high = "#009E73", midpoint = 30, space = "Lab", guide = "colourbar") + theme_minimal() 
         + geom_point(data=Mdat[Mdat$cell==num&Mdat$slice==1,], aes(x=y0_rot*-0.064, y=x0_rot*-0.064),colour="black") +geom_point() + geom_point(data=Mdat[Mdat$cell==num&Mdat$slice==1,], aes(x=y1_rot*-0.064, y=x1_rot*-0.064),colour="black"))
}
