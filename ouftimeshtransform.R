##FIXED REP first row empty problem!
...
Q <- readline("spotfinder analysis? y/n: ")
U <- readline("object analysis? y/n: ")
...
MESH <- ouftimesh(cellList)
save(MESH, file =paste("MESH_", listname, ".Rda", sep=""))
...
M <- ouftiM(cellList, MESH)
save(M, file =paste("M_", listname, ".Rda", sep=""))
...
REP <- ouftispot(cellList)
save(REP, file =paste("spots_", listname, ".Rda", sep=""))
}
...
if(U=="y"){
  ...
  OBJ <- ouftiobject(cellList)
  save(OBJ, file = paste(listname, "OBJ.Rda", sep="_"))
}